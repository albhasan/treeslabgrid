...
test_that("aggregate_vector works with lines", {
  ...
  lin_ls <- lapply(line_vertex, function(x) {
    lon <- sample(seq(x_min + neg_buffer, x_max - neg_buffer, 0.1), size = x)
    lat <- sample(seq(y_min + neg_buffer, y_max - neg_buffer, 0.1), size = x)
    sf::st_linestring(matrix(c(lon, lat), byrow = FALSE, ncol = 2), dim = "XY")
  })
  lines_sf <- sf::st_sf(data.frame(
  ...
  # Lenght of lines aggregated by category should match with the aggregation
  # results.
